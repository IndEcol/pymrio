...
def system_is_balanced(
    Z: pd.DataFrame, VA: pd.DataFrame, Y: pd.DataFrame, x: pd.DataFrame, atol=0.01, report =False
):
    ...
    4) Returns Report if system is not balance
   ...
   # Check shape of Inter industry is square
    if isinstance(Z, pd.DataFrame):
        ...
        if num_rows_z == num_cols_z:
            ...
            raise ValueError("Number of rows does not equal columns")
    else:
        pass
    ...
    for i in range(len(Z)):
        ...
        if np.isclose(total, float(x.iloc[i]), atol=atol):
            pass
        else:
            raise ValueError(
    ...
    if np.isclose(input, output, atol=atol):
        pass
    else:
        raise ValueError(
    ...
    for i in range(len(Z)):
        ...
        if np.isclose(sector_input_sum, sector_output_sum, atol=atol):
            pass
