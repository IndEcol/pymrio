...
def system_is_balanced(Z: pd.DataFrame, VA: pd.DataFrame, Y: pd.DataFrame, x:pd.DataFrame):
    ...
    for i in range(len(Z)):
        ...
        if np.isclose(total,float(x.iloc[i]), atol= 0.01):
            pass       
        else:
    ...
    Z_sum_vertical = Z.sum(axis=0).sum() #Input Matrix sum (vertical sum of InterIndustry)
    ...
    if np.isclose(Input,Output, atol= 0.01):
        pass
    else:
    ...
    for i in range(len(Z)):    
        ...
        if np.isclose(sector_Input_sum,sector_output_sum, atol= 0.01):
            pass
