...
def td_IO_Data_Miller():
    ...
    class IO_Data_Miller():
        ...
        xnew_df = pd.DataFrame(data=xnew_arr,
                               index=['sec1', 'sec2'],
                               columns=['indout'],
                               )
        ...
        labcoeff_df = pd.DataFrame(data=labcoeff_arr,
                                   index=['total labor'],
                                   columns=['sec1', 'sec2'],
                                   )
        ...
        labtot_df = pd.DataFrame(data=labtot_arr,
                                 index=['total labor'],
                                 columns=['sec1', 'sec2'],
                                 )
...
def td_small_MRIO():
    ...
    class IO_Data():
        ...
        A = pd.DataFrame(
                data=[
                    [0.19607843137254902, 0.3333333333333333, 0.017241379310344827, 0.12, 0.09615384615384616, 0.1794871794871795],   # noqa
                    [0.0, 0.13333333333333333, 0.0, 0.0, 0.09615384615384616, 0.07692307692307693],                                   # noqa
                    [0.19607843137254902, 0.2, 0.3448275862068966, 0.08, 0.038461538461538464, 0.0],                                  # noqa
                    [0.09803921568627451, 0.0, 0.0, 0.02, 0.19230769230769232, 0.23076923076923075],                                  # noqa
                    [0.0, 0.6666666666666666, 0.017241379310344827, 0.0, 0.38461538461538464, 0.02564102564102564],                   # noqa
                    [0.09803921568627451, 0.0, 0.0, 0.02, 0.19230769230769232, 0.2564102564102564],                                   # noqa
                    ],
               index=_Z_multiindex,
               columns=_Z_multiindex,
               )
        ...
        L = pd.DataFrame(
                data=[
                    [1.3387146304736708, 0.9689762471208287, 0.05036622549592462, 0.17820960407435948, 0.5752019383714646, 0.4985179148178926],         # noqa
                    [0.02200779585580331, 1.3716472861392823, 0.0076800357678581885, 0.006557415453762468, 0.2698335633228079, 0.15854643902810828],    # noqa
                    [0.43290422861412026, 0.8627066565439678, 1.5492942759220427, 0.18491657196329184, 0.44027825642348534, 0.26630955082840885],       # noqa
                    [0.18799498787612925, 0.5244084722329316, 0.020254008037620782, 1.0542007368783255, 0.5816573175534603, 0.44685014763069275],       # noqa
                    [0.04400982046095892, 1.5325472495862535, 0.05259311578831879, 0.014602513642445088, 1.9545285794951548, 0.2410917825607805],       # noqa
                    [0.19294222439918532, 0.5382086951864299, 0.020787008249137116, 0.05562707205933412, 0.596964089068025, 1.4849251515157111],        # noqa
                    ],
               index=_Z_multiindex,
               columns=_Z_multiindex,
               )
        ...
        S = pd.DataFrame(
                data=[
                    [0.39215686274509803, 0.06666666666666667, 0.7241379310344828, 0.08, 0.38461538461538464, 0.1282051282051282],    # noqa
                    [0.09803921568627451, 0.26666666666666666, 0.1896551724137931, 0.16, 0.038461538461538464, 0.2564102564102564],   # noqa
                    ],
                 index=['ext_type_1', 'ext_type_2'],
                 columns=_Z_multiindex,
                )
        ...
        M = pd.DataFrame(
                data=[
                    [0.896638324101429, 1.7965474933011258, 1.1666796580507097, 0.3013124703668342, 1.4371886818255364, 0.7177624711634629],   # noqa
                    [0.3004620527704837, 0.9052387706892087, 0.311411003349379, 0.23778766312079253, 0.5331560745059157, 0.6031791628984159],  # noqa
                    ],
                 index=['ext_type_1', 'ext_type_2'],
                 columns=_Z_multiindex,
                )
        ...
        D_imp = pd.DataFrame(
                data=[
                    [1.2792573306446737, 10.499069649181388, 1.2752266807875912, 6.604374747509535, 8.364185902326788, 10.842115601865368],  # noqa
                    [2.054905005956757, 3.9151998960771452, 1.522271392125981, 1.743464577633809, 3.6794577757050337, 3.221508682410955],    # noqa
                    ],
                 index=['ext_type_1', 'ext_type_2'],
                 columns=_Z_multiindex,
                )
        ...
        D_exp = pd.DataFrame(
                data=[
                    [8.251832343364468, 0.5304113433404783, 17.02843256499675, 1.3307504334524414, 9.797226856351275, 1.9255763708099365],  # noqa
                    [2.062958085841117, 2.1216453733619134, 4.459827576546767, 2.6615008669048827, 0.9797226856351275, 3.851152741619873],  # noqa
                    ],
                 index=['ext_type_1', 'ext_type_2'],
                 columns=_Z_multiindex,
                )
...
def test_calc_x_df(td_IO_Data_Miller):
    pdt.assert_frame_equal(
            ...
            calc_x(td_IO_Data_Miller.Z_df, td_IO_Data_Miller.fd_df)
            )
...
def test_calc_Z_df(td_IO_Data_Miller):
    pdt.assert_frame_equal(
            td_IO_Data_Miller.Z_df,
            calc_Z(td_IO_Data_Miller.A_df, td_IO_Data_Miller.x_df)
            )
...
def test_calc_A_df(td_IO_Data_Miller):
    pdt.assert_frame_equal(
            td_IO_Data_Miller.A_df,
            calc_A(td_IO_Data_Miller.Z_df, td_IO_Data_Miller.x_df)
            )
...
def test_calc_L_df(td_IO_Data_Miller):
    pdt.assert_frame_equal(
            td_IO_Data_Miller.L_df,
            calc_L(td_IO_Data_Miller.A_df)
            )


def test_calc_L_arr(td_IO_Data_Miller):
    npt.assert_allclose(
            td_IO_Data_Miller.L_arr,
            calc_L(td_IO_Data_Miller.A_arr),
            rtol=1e-5
            )
...
def test_calc_x_MRIO(td_small_MRIO):
    pdt.assert_frame_equal(
            td_small_MRIO.x,
            calc_x(td_small_MRIO.Z, td_small_MRIO.Y)
                )
...
def test_calc_A_MRIO(td_small_MRIO):
    pdt.assert_frame_equal(
            td_small_MRIO.A,
            calc_A(td_small_MRIO.Z, td_small_MRIO.x)
                )
...
def test_calc_S_Y_MRIO(td_small_MRIO):
    pdt.assert_frame_equal(
            td_small_MRIO.S_Y,
            calc_S_Y(td_small_MRIO.F_Y, td_small_MRIO.Y.sum(axis=0)))
...
def test_calc_F_Y_MRIO(td_small_MRIO):
    ...
    pdt.assert_frame_equal(
        td_small_MRIO.F_Y,
        calc_F_Y(S_Y, td_small_MRIO.Y.sum(axis=0)))


def test_calc_M_MRIO(td_small_MRIO):
    pdt.assert_frame_equal(
            td_small_MRIO.M,
            calc_M(td_small_MRIO.S, td_small_MRIO.L)
                )
...
def test_calc_accounts_MRIO(td_small_MRIO):
    ...
    nD_cba, nD_pba, nD_imp, nD_exp = calc_accounts(
            ...
            td_small_MRIO.L,
            td_small_MRIO.Y,
            nr_sectors=len(td_small_MRIO.Z.index.get_level_values(
                'sector').unique()),
            )
    # test all
    pdt.assert_frame_equal(
